...
Connect image-gen-mcp to Claude Desktop as an MCP server.
...
- Ensure `image-gen-mcp` is on your PATH (try running it in a terminal first)
- Restart Claude Desktop completely (quit and reopen)

### Tools not visible

- Check that `IMAGE_GEN_MCP_READ_ONLY` is set to `"false"` -- the `generate_image` tool is hidden in read-only mode (the default)
