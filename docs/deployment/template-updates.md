...
3. **Check the seeded-once files.** Copier's `_skip_if_exists` list names
   ...
   `.claude-plugin/**`, `src/image_generation_mcp/tools.py`,
   `src/image_generation_mcp/resources.py`, `src/image_generation_mcp/prompts.py`,
   `src/image_generation_mcp/domain.py`, `tests/conftest.py`,
   ...
   `src/image_generation_mcp/static/app.src.html`.
