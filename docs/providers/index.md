...
image-gen-mcp supports multiple image generation providers. Each provider is registered at startup based on available configuration (API keys, service URLs).
...
2. **OpenAI** -- registered when `IMAGE_GEN_MCP_OPENAI_API_KEY` is set
3. **A1111** -- registered when `IMAGE_GEN_MCP_A1111_HOST` is set
