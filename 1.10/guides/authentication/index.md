...
When `AUTH_MODE` is not set, it auto-detects: `oidc-proxy` when client credentials are present, `remote` otherwise. See [OIDC deployment guide](https://pvliesdonk.github.io/image-generation-mcp/1.10/deployment/oidc/index.md) for setup details.
...
- [OIDC Authentication reference](https://pvliesdonk.github.io/image-generation-mcp/1.10/deployment/oidc/index.md)
...
- For subpath deployments, see the [subpath deployment guide](https://pvliesdonk.github.io/image-generation-mcp/1.10/deployment/oidc/#subpath-deployments); `BASE_URL` must include the prefix, `HTTP_PATH` must not
...
- **Remote mode** (recommended): requires JWT access tokens. Add `access_token_signed_response_alg: 'RS256'` to the Authelia client registration. See [Authelia setup](https://pvliesdonk.github.io/image-generation-mcp/1.10/deployment/oidc/#setup-with-authelia) for details.
...
**Fix:** Add `token_endpoint_auth_method: 'client_secret_post'` to the Authelia client registration. See [Authelia setup](https://pvliesdonk.github.io/image-generation-mcp/1.10/deployment/oidc/#setup-with-authelia).
