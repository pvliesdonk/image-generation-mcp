...
| `IMAGE_GENERATION_MCP_STYLES_DIR`       | Path | `~/.image-generation-mcp/styles/` | Directory for style preset files (`.md` with YAML frontmatter). Created automatically if it does not exist. See the [Style Library Guide](https://pvliesdonk.github.io/image-generation-mcp/1.10/guides/styles/index.md). |
...
- **`remote`**: when `BASE_URL` + `OIDC_CONFIG_URL` are set but `CLIENT_ID`/`CLIENT_SECRET` are not. Recommended for new deployments — avoids the [OIDCProxy session lifetime issue](https://pvliesdonk.github.io/image-generation-mcp/1.10/guides/authentication/#known-limitations-oidc-session-lifetime).
...
The computation is `sha256(BASE_URL + HTTP_PATH)[:32] + ".claudemcpcontent.com"`. Set `APP_DOMAIN` explicitly only if you need to override this (e.g. for a non-Claude host like ChatGPT, or a custom HTTP path).
