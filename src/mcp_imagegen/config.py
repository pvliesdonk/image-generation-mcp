...
All environment variables share the ``MCP_IMAGEGEN_`` prefix (controlled by
...
# ---------------------------------------------------------------------------
_ENV_PREFIX = "MCP_IMAGEGEN"


def get_log_level() -> int:
    """Return the configured log level from ``MCP_IMAGEGEN_LOG_LEVEL``.
...
_DEFAULT_SCRATCH_DIR = Path.home() / ".mcp-imagegen" / "images"
...
def load_config() -> ServerConfig:
    ...
    - ``MCP_IMAGEGEN_READ_ONLY``: disable write tools; default ``true``.
    - ``MCP_IMAGEGEN_SCRATCH_DIR``: image save directory.
    - ``MCP_IMAGEGEN_OPENAI_API_KEY``: OpenAI API key.
    - ``MCP_IMAGEGEN_A1111_HOST``: A1111 WebUI URL.
    - ``MCP_IMAGEGEN_A1111_MODEL``: A1111 checkpoint name for preset detection.
    - ``MCP_IMAGEGEN_DEFAULT_PROVIDER``: default provider; default ``"auto"``.
